...
type GKMCacheSpec struct {
	...
	// podTemplate is an optional field that allows customizing the Pod used in the
	// Job that GKM launches to extract the GPU Kernel Cache to a PVC. This field
	// is used to apply any Tolerations, NodeSelectors, custom Labels or Affinity
	// settings needed to allow the Job to be launched on the same node as the
	// application pod that will run.
...
type PodTemplate struct {
	// metadata defines the optional fields usually contained by a metadata
	// structure, like labels and annotations, that may need to allow the GKM
	// launched Job to run on the same node as the application pod that will run.
	Metadata *TemplateMetadata `json:"metadata,omitempty"`

	// spec defines the optional fields that may be needed to allow the GKM
	// launched Job to run on the same node as the application pod that will run.
...
type PodTemplateSpec struct {
	...
	// tolerations is an optional selector which allows the GKM launched Job to
	// to have its pods scheduled on nodes that have a taint applied. The
	// toleration allows the taint to be bypassed.
	Tolerations []corev1.Toleration `json:"tolerations,omitempty"`

	// affinity is an optional selector which allows the GKM launched Job to
	// to have its pods scheduled based on the entered labels.
	Affinity *corev1.Affinity `json:"affinity,omitempty"`

	// priorityClassName is an optional field. If specified, it indicates the pod's
	// priority. "system-node-critical" and "system-cluster-critical" are two
	// special keywords which indicate the highest priorities with the former being
...
type TemplateMetadata struct {
	// labels are optional and will be applied to the pods associated with the GKM
	// launched Job that is responsible for extracting the GPU Kernel Cache. labels
	// are a map of string keys and values that can be used to organize and
	// categorize (scope and select) objects. May match selectors of replication
	// controllers and services. More info:
	// https://kubernetes.io/docs/concepts/overview/working-with-objects/labels
	Labels map[string]string `json:"labels,omitempty"`

	// annotations are optional and will be applied to the pods associated with the
	// GKM launched Job that is responsible for extracting the GPU Kernel Cache.
