...
if [[ "$1" == "help" || "$1" == "-help" || "$1" == "--help" ]]; then
    ...
    echo "  <SCOPE> is \"namespace\", \"ns\", \"cluster\" or \"cl\" and required."
    ...
    echo "  ./generate-files.sh rox ns rocm v3"
    echo ""
    exit 0
fi
...
# Lock so only one instance of script is run at a time
LOCK_DIR=.gkm-generate-files.exclusivelock
if ! mkdir ${LOCK_DIR} &>/dev/null; then
    echo "Could not get lock, retry"
...
trap 'rmdir ${LOCK_DIR} &>/dev/null; exit' INT TERM EXIT
...
cat <<EOF > ${OVERLAY_DIR_SCOPE}/kustomization.yaml
...
if [[ "$CUSTOM_AFFINITY" != "" ]]; then
    if [[ -r "${CUSTOM_AFFINITY}" ]]; then
        AFFINITY_INSTANCE=$(cat "${CUSTOM_AFFINITY}") || { echo "Error: Failed to read file - CUSTOM_AFFINITY=${CUSTOM_AFFINITY}" >&2; exit 1; }
    else
        echo "Error: File does not exist - CUSTOM_AFFINITY=${CUSTOM_AFFINITY}"
        exit 1
    fi
elif [[ "$ENVIRONMENT" == "nfd" ]]; then
...
if [[ "$CUSTOM_TOLERATION" != "" ]]; then
    if [[ -r "${CUSTOM_TOLERATION}" ]]; then
        TOLERATION_INSTANCE=$(cat "${CUSTOM_TOLERATION}") || { echo "Error: Failed to read file - CUSTOM_TOLERATION=${CUSTOM_TOLERATION}" >&2; exit 1; }
    else
        echo "Error: File does not exist - CUSTOM_TOLERATION=${CUSTOM_TOLERATION}"
        exit 1
    fi
...
if [[ "$CUSTOM_NODE_SELECTOR_1" != "" ]]; then
    if [[ -r "${CUSTOM_NODE_SELECTOR_1}" ]]; then
        ...
        echo "Error: File does not exist - CUSTOM_NODE_SELECTOR_1=${CUSTOM_NODE_SELECTOR_1}"
        exit 1
    fi
fi
...
if [[ "$CUSTOM_NODE_SELECTOR_2" != "" ]]; then
    if [[ -r "${CUSTOM_NODE_SELECTOR_2}" ]]; then
        NODE_SELECTOR_INSTANCE_2=$(cat "${CUSTOM_NODE_SELECTOR_2}") || { echo "Error: Failed to read file - CUSTOM_NODE_SELECTOR_2=${CUSTOM_NODE_SELECTOR_2}" >&2; exit 1; }
    else
        echo "Error: File does not exist - CUSTOM_NODE_SELECTOR_2=${CUSTOM_NODE_SELECTOR_2}"
        exit 1
    fi
...
if [[ "$CUSTOM_NODE_SELECTOR_3" != "" ]]; then
    if [[ -r "${CUSTOM_NODE_SELECTOR_3}" ]]; then
        NODE_SELECTOR_INSTANCE_3=$(cat "${CUSTOM_NODE_SELECTOR_3}") || { echo "Error: Failed to read file - CUSTOM_NODE_SELECTOR_3=${CUSTOM_NODE_SELECTOR_3}" >&2; exit 1; }
    else
        echo "Error: File does not exist - CUSTOM_NODE_SELECTOR_3=${CUSTOM_NODE_SELECTOR_3}"
        exit 1
    fi
elif [[ "$ENVIRONMENT" == "kind" && "$ACCESS" == "rwo" ]]; then
