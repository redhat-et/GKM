...
// ClusterGKMCacheOperatorReconciler reconciles a ClusterGKMCacheNode object
type ClusterGKMCacheOperatorReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Logger logr.Logger

	currCache     *gkmv1alpha1.ClusterGKMCache
	currCacheNode *gkmv1alpha1.ClusterGKMCacheNode
}
...
// the ClusterGKMCacheNode object against the actual cluster state, and then
...
func (r *ClusterGKMCacheOperatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	r.Logger.Info("Enter Operator ClusterGKMCache Reconcile", "Name", req)
	...
	if err := r.List(ctx, cacheList, opts...); err != nil {
		r.Logger.Error(err, "failed to list ClusterGKMCache", "Namespace", req.Namespace, "Name", req.Name)
		return ctrl.Result{Requeue: true, RequeueAfter: utils.RetryAgentFailure},
			fmt.Errorf("failed getting list of ClusterGKMCache for full reconcile %s : %v",
				req.NamespacedName,
				err)
	}
	...
	if len(cacheList.Items) == 0 {
		...
		r.Logger.Info("ClusterGKMCache Status Controller found no caches")
	...
	for cacheIndex := range cacheList.Items {
		...
		r.Logger.Info("Reconciling ClusterGKMCache", "Namespace", r.currCache.Namespace, "Name", r.currCache.Name)
		...
		// Call KubeAPI to Retrieve the list of ClusterGKMCacheNodes.
		...
		if err != nil {
			...
			r.Logger.Error(err, "failed to get ClusterGKMCacheNode List", "Namespace", r.currCache.Namespace, "Name", r.currCache.Name)
			errorHit = true
			continue
		}
		...
		r.Logger.Info("Processed ClusterGKMCache",
			"Namespace", r.currCache.Namespace,
		...
		if r.isCacheBeingDeleted() {
			if totalNodeCnt == 0 {
				// Everything should be cleaned up, so delete the ClusterGKMCacheNode specific
				// finalizer from the ClusterGKMCache.
				if changed := controllerutil.RemoveFinalizer(r.currCache, r.getCacheFinalizer()); changed {
					r.Logger.Info("Calling KubeAPI to delete ClusterGKMCacheNode Finalizer from ClusterGKMCache",
						"Namespace", r.currCache.Namespace,
						"CacheNodeName", r.currCache.Name,
					)
					err := r.Update(ctx, r.currCache)
					if err != nil {
						r.Logger.Error(err, "failed to delete ClusterGKMCacheNode Finalizer from ClusterGKMCache")
						errorHit = true
						continue
					}
					// ClusterGKMCache object was updated. Return and change will retrigger a new reconcile.
					return ctrl.Result{Requeue: false}, nil
				}
			} else {
				r.Logger.Info("Deleting ClusterGKMCache still in progress",
					"Namespace", r.currCache.Namespace,
					"CacheName", r.currCache.Name,
...
func (r *ClusterGKMCacheOperatorReconciler) isCacheBeingDeleted() bool {
	return !r.currCache.GetDeletionTimestamp().IsZero()
}

// getCacheFinalizer returns the finalizer that is added to the ClusterGKMCache object.
