...
type GKMCacheNodeReconciler struct {
	client.Client
...
func (r *GKMCacheNodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx)

	var nodeStatus gkmv1alpha1.GKMCacheNode
	if err := r.Get(ctx, req.NamespacedName, &nodeStatus); err != nil {
		logger.Error(err, "unable to fetch GKMCacheNodeStatus")
...
func (r *GKMCacheNodeReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&gkmv1alpha1.GKMCacheNode{}).
		Complete(r)
}
...
func setNodeCondition(obj *gkmv1alpha1.GKMCacheNode, condType string, status metav1.ConditionStatus, reason, msg string) {
