...
!!! Note
    This document is outdated and not been updated since removing CSI
    Agent and moving to PVC Storage.

## Introduction

The GPU Kernel Manager (GKM) Operator and Agent manage GPU kernel cache
distribution and usage within a Kubernetes environment.
The Operator validates and inspects GPU kernel images, and handles Kernel
cache extraction from OCI Images at the global level.
The GKM Agent runs on each node, handles Kernel cache extraction from OCI
Images at the node level, and tracks pod usage of the extracted kernel cache.
The architecture follows a controller-runtime approach, leveraging Kubernetes
Custom Resources (CRs).

## Motivation

The primary motivation of GPU Kernel Manager is to reduce the startup time of
large language models (LLMs) that use GPU Kernels.
